...
task('dd', async (ctx) => {
  ...
  let stdout = p.stdout as string
  logger.debug('p.stdout.trim() === ctx.cwd()', stdout === ctx.cwd)
  logger.debug(stdout.endsWith('src'))
})
...
task('ff', [dep('aa').options({ aa: 1 }), dep('aa').options({ aa: 2 })])

const noop = (f) => f
task('notForce', [dep('aa').options({ noop }), dep('aa').options({ noop })])
task('force', [dep('aa').options({ noop }).force(), dep('aa').options({ noop }).force()])

task<{ t: number }>('wait', async (ctx) => {
  await new Promise((res) => setTimeout(res, ctx.options.t))
  console.log('wait', ctx.options.t)
})
task('sync', [dep('wait').options({ t: 100 }), dep('wait').options({ t: 1 })])

task('async', [dep('wait').async().options({ t: 100 }), dep('wait').async().options({ t: 1 })])
task('async:priority', [
...
task('pushpopd', async (ctx) => {
  let pwd = (
    await ctx.env('NODE_ENV', 'TEST').env('SOME_ENV', 'SOME').exec('pwd', { stdio: 'pipe' })
  ).stdout as string
